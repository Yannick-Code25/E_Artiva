...
exports.registerUser = async (req, res) => {
  ...
  if (!name || !email || !password) {
    return res.status(400).json({ message: 'Veuillez fournir le nom, l\'email et le mot de passe.' });
  }
...
exports.loginUser = async (req, res) => {
  ...
  try {
    ...
    const adminQuery = 'SELECT id, email, password_hash, role FROM admin WHERE email = $1';
    ...
    if (adminResult.rows.length > 0) {
      ...
      const userQuery = 'SELECT id, email, password_hash, role, name, address, phone FROM users WHERE email = $1';
    ...
    res.status(200).json({
      message: `Connexion réussie en tant que ${userTable === 'admin' ? 'administrateur' : 'client'} !`,
      token,
...
exports.registerAdmin = async (req, res) => {
  ...
  // Validation basique
  if (!name || !email || !password) {
    return res.status(400).json({ message: 'Veuillez fournir le nom, l\'email et le mot de passe pour l\'admin.' });
  }
