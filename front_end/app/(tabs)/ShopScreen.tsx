...
export default function TabShopScreen() {
    ...
    try {
      ...
      setAllProducts(
        productsArray.map((p: any) => ({
          ...
          price: `${p.price !== undefined && p.price !== null ? parseFloat(String(p.price)).toFixed(2) : "N/A"} FCFA`,
          imageUrl: p.image_url || `https://via.placeholder.com/150x150/E0E0E0/909090?text=${encodeURIComponent(p.name || "Prod")}`,
          stock: p.stock,
  ...
  const subCategories = useMemo(() => {
    ...
    console.log("ShopScreen: allCategories", JSON.stringify(allCategories, null, 2));
  ...
  const handleProductPress = (productId: string | number) => {
   ...
   useFocusEffect(
        useCallback(() => {
            return () => {
                console.log("ShopScreen: L'écran a perdu le focus, réinitialisation des états.");
                setSearchTerm("");
...
//     let productsUrl = `${API_BASE_URL}/products`;
//     if (selectedSubCategoryId) {
//       productsUrl += `?category_id=${selectedSubCategoryId}`;
//     } else if (selectedMainCategoryId && subCategories.length === 0) {
//       productsUrl += `?category_id=${selectedMainCategoryId}`;
//     }
...
//       );
//       console.log("ShopScreen: Catégories chargées:", catData.length);
//       console.log("ShopScreen: allCategories", JSON.stringify(catData, null, 2));
...
//         productsArray.length
//       );
//       console.log("ShopScreen: allProducts", JSON.stringify(productsArray, null, 2));
//     } catch (err: any) {
//       console.error("ShopScreen: Erreur fetchData:", err.message, err);
//       setError(err.message || "Erreur de chargement des données.");
//       setAllCategories([]);
...
//                   refreshControl={
//                     <RefreshControl
//                       refreshing={isLoading && initialLoadComplete} // Affiche le spinner si on charge ET que le premier chargement est fait
//                       onRefresh={onRefresh}
//                       tintColor={tintColor}
//                     />
//                   }
