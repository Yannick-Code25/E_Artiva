...
import { View, Text, StyleSheet, Image, ScrollView, TouchableOpacity, FlatList, ActivityIndicator, TextInput, Button, Alert} from 'react-native';
...
export default function TabShopScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setAllCategories(catData.map((c: any) => ({ ...c, id: String(c.id), parent_id: c.parent_id ? String(c.parent_id) : null })));
      console.log("ShopScreen: Catégories chargées:", catData.length);

      if (!prodResponse.ok) throw new Error(`Erreur produits (${prodResponse.status})`);
      ...
      setAllProducts(prodData.map((p: any) => ({ 
        ...
        price: `${(p.price !== undefined && p.price !== null ? String(p.price) : 'N/A')} FCFA`,
        imageUrl: p.image_url || `https://via.placeholder.com/150x150/?text=${encodeURIComponent(p.name || 'Prod')}`, // ESSENTIEL
        category_ids: p.category_ids || [], // VIENT DE L'API MAINTENANT
        categories_names: p.categories_names || [],
        tags_names: p.tags_names || [],
    })));
      console.log("ShopScreen: Produits chargés:", prodData.length);
  ...
  const getProductsToDisplay = (): Product[] => {
    ...
    if (selectedSubCategoryId) {
      ...
      productsToFilter = allProducts.filter(prod => 
        prod.category_ids && prod.category_ids.map(id => String(id)).includes(String(selectedSubCategoryId))
      );
      console.log(`Produits trouvés pour sous-cat ${selectedSubCategoryId}:`, productsToFilter.length);
    } else if (selectedMainCategoryId) {
      const subs = getSubCategories(); // getSubCategories doit être défini et fonctionner
      if (subs.length === 0) { // Si la catégorie principale n'a pas de sous-catégories
        console.log(`Filtrage pour catégorie principale SANS sous-cat ID: ${selectedMainCategoryId}`);
        productsToFilter = allProducts.filter(prod => 
            prod.category_ids && prod.category_ids.map(id => String(id)).includes(String(selectedMainCategoryId))
        );
        console.log(`Produits trouvés pour cat principale ${selectedMainCategoryId}:`, productsToFilter.length);
      } else {
        productsToFilter = []; // Aucune sous-catégorie sélectionnée, donc pas de produits affichés pour l'instant
        console.log("Catégorie principale sélectionnée, mais en attente de sélection de sous-catégorie.");
      }
    } else {
      ...
      console.log("Aucune catégorie sélectionnée pour l'affichage des produits.");
    }
    ...
    if (searchTerm.trim() !== '') {
      ...
      console.log(`Produits après recherche "${searchTerm}":`, productsToFilter.length);
    }
  ...
  // --- Affichage principal ---
  if (isLoadingCategories) { // Loader initial pour les catégories
    return (
      <View style={styles.centered}>
        <ActivityIndicator size="large" color={Colors[colorScheme ?? 'light'].tint} />
        <Text style={{marginTop: 10, color: Colors[colorScheme ?? 'light'].text}}>Chargement des catégories...</Text>
      </View>
    );
  }
  ...
  if (error) {
    return (
      <View style={styles.centered}>
        ...
        <Button title="Réessayer" onPress={fetchData} color={Colors[colorScheme ?? 'light'].tint}/>
      </View>
    );
  }
        ...
        <View style={styles.rightPane}>
          ...
          {(selectedSubCategoryId || (selectedMainCategoryId && subCategories.length === 0)) && (
            <>
              ...
              {isLoadingProducts ? (
                <ActivityIndicator style={{marginTop: 20}} size="large" color={Colors[colorScheme ?? 'light'].tint} />
                ...
                <Text style={styles.noProductsText}>
                    {searchTerm ? "Aucun produit ne correspond à votre recherche." : "Aucun produit dans cette catégorie."}
                </Text>
              )}
            </>
          )}
          ...
          {!selectedMainCategoryId && <Text style={styles.placeholderText}>Sélectionnez une catégorie à gauche.</Text>}
          {selectedMainCategoryId && subCategories.length > 0 && !selectedSubCategoryId && productsToDisplay.length === 0 &&
            <Text style={styles.placeholderText}>Sélectionnez une sous-catégorie pour voir les produits.</Text>
          }
...
const styles = StyleSheet.create({
  ...
  productImage: { // Si tu utilises une vraie image
    width: '100%',
    aspectRatio: 1,
    borderRadius: 6,
    marginBottom: 8,
