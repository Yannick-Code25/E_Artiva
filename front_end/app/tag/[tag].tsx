...
const API_BASE_URL = "http://192.168.244.128:3001/api"; // **TON IP**
  ...
  const fetchProductsByTag = useCallback(async () => {
    ...
    try {
      ...
      const adaptedProducts = productsData.map((prod: any) => {
        const productName =
          prod.name || t("tagScreen.unknownProduct", "Produit Inconnu");
        const productPrice =
      ...
      setError(
        t(
          ...
          "Impossible de charger les produits pour ce tag."
        )
      );
      setProducts([]);
    } finally {
