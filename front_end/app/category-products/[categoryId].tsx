...
import ProductCard, { Product as ProductType } from '../../components/ProductCard'; // Réutilise ton ProductCard
...
export default function CategoryProductsScreen() {
  ...
  const [pageTitle, setPageTitle] = useState(categoryName || `Catégorie ${categoryId}`);
  ...
  const fetchProductsByCategory = useCallback(async () => {
    ...
    console.log(`CategoryProductsScreen: Fetching products for category ID: ${categoryId}`);
    setIsLoading(true);
    setError(null);
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Erreur chargement produits pour catégorie (${response.status})`);
      }
      ...
      console.log(`CategoryProductsScreen: ${adaptedProducts.length} produits chargés pour la catégorie ${categoryId}`);
      ...
      if (!categoryName && adaptedProducts.length > 0 && adaptedProducts[0].categories_names) {
      ...
      setError(err.message || 'Impossible de charger les produits pour cette catégorie.');
      setProducts([]);
    } finally {
      setIsLoading(false);
    }
  ...
  if (error) {
    return <View style={styles.centered}><Text style={{color:'red'}}>{error}</Text><Button title="Réessayer" onPress={fetchProductsByCategory} color={tintColor}/></View>;
  }
    ...
    <View style={[styles.screenContainer, {backgroundColor}]}>
      ...
      {products.length === 0 ? (
        <View style={styles.centered}>
          <Text style={{color: textColor, fontSize: 16}}>Aucun produit trouvé dans cette catégorie.</Text>
        </View>
      ) : (
        <FlatList
          data={products}
