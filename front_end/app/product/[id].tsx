...
export default function ProductDetailScreen() {
  ...
  const fetchProductDetails = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "ProductDetailScreen: Données brutes API:",
      ...
      if (dataFromApi.images && dataFromApi.images.length > 0) {
        ...
        // Fallback sur main_image_url de la table products
      ...
      console.log(
        "ProductDetailScreen: Details fetched and processed:",
        processedData
      );
    } catch (err: any) {
  ...
  // Vérifier si le produit actuel est dans la wishlist
  ...
  const handleWishlistToggleOnDetail = () => {
    ...
    if (isInWishlist) {
      ...
      addToWishlist(product); // product est de type ProductDetail qui étend BaseProductType
    }
  };
  ...
  const handleQuantityChangeOnDetailPage = (amount: number) => {
    if (!product) return;
    const currentQuantityInCart = cartItemForThisProduct
      ? cartItemForThisProduct.quantity
      : 0;
...
const styles = StyleSheet.create({
  ...
  carouselWrapper: {
    /* Hauteur implicite ou à définir */
  },
  ...
  carouselImage: {
    width: "100%",
    aspectRatio: 1.1 /* Ratio pour images un peu plus hautes que larges */,
  },
