...
export function WishlistProvider({ children }: { children: ReactNode }) {
  ...
  const loadWishlist = useCallback(async () => {
    ...
    try {
      if (userToken) {
        console.log("WishlistContext: Utilisateur connecté, chargement depuis l'API.");
        const response = await fetch(`${API_BASE_URL}/wishlist`, {
          headers: { 'Authorization': `Bearer ${userToken}` },
        });
        if (!response.ok) throw new Error('Erreur API chargement wishlist');
        const data: WishlistItem[] = await response.json();
        setWishlistItems(data.map(item => ({...item, id: String(item.id)}))); // S'assurer que l'id est string
        await AsyncStorage.setItem(WISHLIST_STORAGE_KEY, JSON.stringify(data)); // Synchroniser avec local
        console.log("WishlistContext: Wishlist chargée depuis API et synchronisée localement:", data.length);
      } else {
        console.log("WishlistContext: Utilisateur non connecté, chargement depuis AsyncStorage.");
  ...
  const addToWishlistAPI = async (product: BaseProductType) => {
    if (!userToken) {
      Alert.alert("Connexion requise", "Veuillez vous connecter pour ajouter des articles à votre liste de souhaits.");
    ...
    try {
      ...
      if (!response.ok && response.status !== 200) { // 200 si déjà dedans
      ...
      Alert.alert("Erreur", "Impossible d'ajouter à la liste de souhaits pour le moment.");
      return false;
    }
  };
  ...
  const removeFromWishlistAPI = async (productId: string | number) => {
    if (!userToken) {
        Alert.alert("Connexion requise", "Veuillez vous connecter pour modifier votre liste de souhaits.");
        return false;
    }
    try {
      ...
      Alert.alert("Erreur", "Impossible de retirer de la liste de souhaits pour le moment.");
      return false;
    }
  };
  ...
  const addToWishlist = async (product: BaseProductType) => {
    ...
    setWishlistItems(prevItems => {
      if (prevItems.find(item => String(item.id) === String(product.id))) return prevItems; // Déjà dedans
      const updatedWishlist = [...prevItems, newItem];
      if (!userToken) AsyncStorage.setItem(WISHLIST_STORAGE_KEY, JSON.stringify(updatedWishlist));
      return updatedWishlist;
    });
    
    if (userToken) {
      const success = await addToWishlistAPI(product);
      if (!success) { // Revert si l'API échoue
        setWishlistItems(prevItems => prevItems.filter(item => String(item.id) !== String(product.id)));
  ...
  const removeFromWishlist = async (productId: string | number) => {
    ...
    setWishlistItems(prevItems => {
        const updatedWishlist = prevItems.filter(item => String(item.id) !== stringProductId);
        if (!userToken) AsyncStorage.setItem(WISHLIST_STORAGE_KEY, JSON.stringify(updatedWishlist));
        return updatedWishlist;
    });
