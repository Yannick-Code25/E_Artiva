...
interface CartContextType {
  ...
  updateQuantity: (identifier: string | number, newQuantity: number) => Promise<void>; // Idem
...
export function useCart(): CartContextType {
  ...
  if (!context) {
    throw new Error('useCart must be used within an AuthProvider and CartProvider');
  }
  return context;
}
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const fetchCartFromServer = useCallback(async () => {
    if (!userToken || !user) { // Si pas de token/user, on ne fait rien ici (le useEffect pour invité gérera)
      setIsLoadingCart(false); // Important de le mettre à false pour les invités
      return;
    }

    console.log("CartContext: Utilisateur connecté, chargement du panier depuis l'API...");
    ...
    try {
      ...
      const adaptedItems = (data.items || []).map((item: any) => ({
          ...
          imageUrl: item.imageUrl || `https://via.placeholder.com/80?text=${encodeURIComponent(item.name||'Img')}`,
      }));
      ...
      console.log("CartContext: Panier chargé depuis API:", adaptedItems.length, "items.");
    } catch (e: any) {
      console.error("CartContext: Erreur dans fetchCartFromServer", e.message);
      Alert.alert("Erreur Panier", "Impossible de synchroniser votre panier avec le serveur.");
  ...
  useEffect(() => {
    const initialLoad = async () => {
      if (userToken && user) {
        ...
        console.log("CartContext: Utilisateur non connecté, chargement depuis AsyncStorage.");
        setIsLoadingCart(true);
        try {
          ...
          console.error("CartContext: Erreur chargement panier invité AsyncStorage", e);
          setCartItems([]);
        } finally {
          setIsLoadingCart(false);
        }
  ...
  useEffect(() => {
    if (!userToken && !isLoadingCart) { // Après le chargement initial
      console.log("CartContext: Utilisateur non connecté, sauvegarde du panier invité dans AsyncStorage.");
      AsyncStorage.setItem(GUEST_CART_STORAGE_KEY, JSON.stringify(cartItems))
        .catch(e => console.error("CartContext: Erreur sauvegarde panier invité AsyncStorage", e));
    }
  }, [cartItems, userToken, isLoadingCart]);


  const addToCart = async (product: BaseProductType, quantityToAdd: number = 1) => {
    console.log("CartContext: addToCart pour produit ID:", product.id, "Quantité:", quantityToAdd);
    if (product.stock !== undefined && product.stock <= 0 && quantityToAdd > 0) {
    ...
    if (userToken && user) {
      ...
      try {
        ...
        if (product.stock !== undefined && newQuantity > product.stock) {
            Alert.alert("Stock insuffisant", `Vous ne pouvez pas ajouter plus de ${product.stock} unités de ce produit.`);
            setIsLoadingCart(false);
            return;
        }
        ...
        const response = await fetch(`${API_BASE_URL}/cart/items`, {
          ...
          body: JSON.stringify({ productId: product.id, quantity: newQuantity }), // Envoyer la quantité totale
        });
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({message: `Erreur HTTP ${response.status}`}));
          throw new Error(errorData.message || "Erreur API ajout/MAJ au panier");
        }
        await fetchCartFromServer(); // Re-synchroniser avec le serveur
      } catch (e: any) {
        console.error("CartContext: Erreur API addToCart", e.message);
        Alert.alert("Erreur Panier", e.message || "Impossible d'ajouter l'article au panier.");
      } finally {
        setIsLoadingCart(false);
      }
    } else { // Utilisateur non connecté
      setCartItems(prevItems => {
        ...
        if (existingItemIndex > -1) {
          ...
          const newQuantity = Math.min(updatedItems[existingItemIndex].quantity + quantityToAdd, product.stock || Infinity);
  ...
  const updateQuantity = async (productId: string | number, newQuantity: number) => {
    console.log(`CartContext: updateQuantity pour ID Produit: ${productId}, nouvelle qté: ${newQuantity}`);
    const stringProductId = String(productId);
    const itemInCart = cartItems.find(item => String(item.id) === stringProductId);
    ...
    if (itemInCart.stock !== undefined && newQuantity > itemInCart.stock) {
        Alert.alert("Stock insuffisant", `Il ne reste que ${itemInCart.stock} unités pour ${itemInCart.name}.`);
        // Ne pas mettre à jour la quantité au-delà du stock
        setCartItems(prev => prev.map(item => item.id === stringProductId ? {...item, quantity: itemInCart.stock || 1} : item));
        return;
    }

    if (userToken && user) {
      if (newQuantity <= 0) { // Si la qté est 0 ou moins, on supprime l'item
        ...
        try {
          ...
          const response = await fetch(`${API_BASE_URL}/cart/items`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${userToken}` },
            body: JSON.stringify({ productId: stringProductId, quantity: newQuantity }),
          });
          if (!response.ok) throw new Error("Erreur API MàJ qté");
          await fetchCartFromServer();
        } catch (e: any) {
          console.error("CartContext: Erreur API updateQuantity", e.message);
          Alert.alert("Erreur Panier", e.message || "Impossible de mettre à jour la quantité.");
        } finally {
          setIsLoadingCart(false);
        }
      }
      ...
      setCartItems(prevItems => {
        if (newQuantity <= 0) {
          return prevItems.filter(item => String(item.id) !== stringProductId);
        }
  ...
  const removeFromCart = async (identifier: string | number) => {
    ...
    console.log(`CartContext: removeFromCart pour ID/cartItemID: ${identifier}`);
    if (userToken && user) {
      const itemToRemove = cartItems.find(item => String(item.cartItemId) === String(identifier) || String(item.id) === String(identifier));
      const cartItemIdToRemove = itemToRemove?.cartItemId;

      if (!cartItemIdToRemove) {
        console.warn("CartContext: cartItemId manquant pour suppression API, suppression locale uniquement.");
        setCartItems(prev => prev.filter(i => String(i.id) !== String(identifier)));
        return;
      }
      setIsLoadingCart(true);
      try {
        const response = await fetch(`${API_BASE_URL}/cart/items/${cartItemIdToRemove}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${userToken}` },
        });
        ...
        Alert.alert("Erreur Panier", e.message || "Impossible de supprimer l'article.");
      } finally {
        setIsLoadingCart(false);
      }
    } else { // Non connecté
      setCartItems(prevItems => prevItems.filter(item => String(item.id) !== String(identifier)));
    }
  };

  const clearCart = async () => { /* ... (logique similaire à removeFromCart pour tous les items, ou appel à API DELETE /cart) ... */ 
    ...
    if (userToken && user) {
        ...
        try {
            ...
            Alert.alert("Erreur Panier", e.message || "Impossible de vider le panier.");
        } finally {
            setIsLoadingCart(false);
        }
    } else { 
  ...
  const getTotalPrice = (): number => {
    return parseFloat(cartItems.reduce((sum, item) => {
  ...
  const value = {
    ...
    getTotalItems, getTotalPrice, isLoadingCart, fetchCartFromServer: fetchCartFromServer,
  };
