...
interface CartContextProps {
  ...
  getTotalPrice: () => number;
  getTotalItems: () => number;
  isLoadingCart: boolean; // État de chargement ajouté
}
...
const CartContext = createContext<CartContextProps>({
  ...
  getTotalPrice: () => 0,
  getTotalItems: () => 0,
  isLoadingCart: false, // État de chargement initialisé à false
});
...
const CartProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>([]);
  const [isLoadingCart, setIsLoadingCart] = useState(true); // État de chargement
  ...
  // Mettre à jour AsyncStorage à chaque modification du panier
  useEffect(() => {
  ...
  const updateQuantity = useCallback((itemId: string, quantity: number) => {
    setCartItems((prevItems) => {
      ...
      if (existingItem.quantity === quantity) {
        // Si la quantité n'a pas changé, ne pas mettre à jour l'état
        return prevItems;
      }

      if (quantity <= 0) {
        // Supprimer l'article si la quantité est inférieure ou égale à zéro
  ...
  const getTotalPrice = useCallback(() => {
    return cartItems.reduce((total, item) => {
      ...
      return isNaN(priceNumber) ? total : total + priceNumber * item.quantity;
    }, 0);
  }, [cartItems]);
